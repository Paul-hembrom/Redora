...
const replacement = `      let totalChunks = 0;
      const chunks: any[] = [];
      let i = 0;
...
const oldCodeRegex = /      let totalChunks = 0;[\s\S]*?        \} catch \(e\) \{[\s\S]*?        \}/;
const match = code.match(oldCodeRegex);
if (match) {
   code = code.replace(oldCodeRegex, replacement);
   fs.writeFileSync('src/components/ReadAloudButton.tsx', code);
   console.log('patched read aloud play chunks successfully');
} else {
   console.log('could not find old code match');
}
