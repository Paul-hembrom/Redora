...
const startIndex = content.indexOf(`  const audioUrl = \`data:audio/wav;base64,\${data.audio_base64}\`;`);
...
if (startIndex === -1 || endIndex === -1) {
    ...
    const lengthToReplace = endIndex + `  return {
    audioUrl,
    timestamps: mappedTimestamps
  };`.length - startIndex;
    
    const sliceToReplace = content.substring(startIndex, startIndex + lengthToReplace);
    content = content.replace(sliceToReplace, newStr);
