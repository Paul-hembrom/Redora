  ...
  if (chapterResults.length === 1) {
    ...
    if (topics.length > 20 && topics.some(t => /unit|chapter/i.test(t.title))) {
      ...
      try {
        ...
        let aiExtracted: any[] | null = null;
        let attempts = 0;
        let lastResult: any[] | null = null;

        while (attempts < 3) {
          attempts++;
          let textToPass = processedText;
          if (attempts > 1) {
            textToPass += "\n\nIMPORTANT: Ensure you return ALL chapters. The previous attempt missed several.";
          }
          
          onProgress(`AI Extraction attempt ${attempts}/3...`);
          aiExtracted = await extractViaAI(textToPass, estimatedChapterCount, fileExtension);
          
          const actualCount = aiExtracted ? aiExtracted.length : 0;
          let isComplete = false;
          
          if (estimatedChapterCount > 5) {
             isComplete = (actualCount >= estimatedChapterCount * 0.8) && (actualCount >= 3);
          } else {
             isComplete = (actualCount >= estimatedChapterCount * 0.8);
          }
          
          if (isComplete) {
            console.log(`[Retry Loop] Attempt ${attempts} successful. Expected: ${estimatedChapterCount}, Got: ${actualCount}`);
            break;
          }
          
          console.warn(`[Retry Loop] Attempt ${attempts} incomplete. Expected: ${estimatedChapterCount}, Got: ${actualCount}`);
          
          if (attempts > 1 && aiExtracted && lastResult && aiExtracted.length === lastResult.length) {
            const sameTitles = aiExtracted.every((ch, i) => ch.title === lastResult![i]?.title);
            if (sameTitles) {
              console.warn("[Retry Loop] Identical incomplete result returned. Breaking retry loop early.");
              break;
            }
          }
          lastResult = aiExtracted;
          aiExtracted = null;
        }

        if (!aiExtracted || aiExtracted.length === 0) {
          console.warn('Extraction incomplete after 3 attempts — falling back to split‑retry.');
          onProgress('Extraction incomplete. Falling back to split-retry...');
          
          const half = Math.floor(processedText.length / 2);
          const firstHalf = processedText.substring(0, half);
          const secondHalf = processedText.substring(half);
          
          const [res1, res2] = await Promise.all([
            extractViaAI(firstHalf, Math.ceil(estimatedChapterCount / 2), fileExtension),
            extractViaAI(secondHalf, Math.floor(estimatedChapterCount / 2), fileExtension)
          ]);
          
          const merged = [];
          if (res1) merged.push(...res1);
          if (res2) merged.push(...res2);
          
          if (merged.length > 0) {
            aiExtracted = merged;
          } else {
            console.warn('Split-retry failed. Falling back to regex pipeline.');
            onProgress('Split-retry failed. Falling back to regex pipeline...');
            
            const outline = await generateOutline(processedText);
            let regexExtracted: Chapter[] = [];
            if (outline && outline.length > 0) {
              regexExtracted = await extractByOutline(processedText, outline);
            }
            if (regexExtracted && regexExtracted.length > 0) {
               chapterResults.length = 0;
               chapterResults.push(...regexExtracted);
            } else {
               const chunks = splitIntoChaptersEnhanced(processedText);
               chapterResults.length = 0;
               chapterResults.push(...chunks.map((ch, i) => ({
                 id: uuidv4(),
                 chapterNumber: i + 1,
                 title: ch.title,
                 summary: '',
