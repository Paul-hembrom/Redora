      ...
      >
        <div className="flex items-center gap-2 truncate">
          ...
          <span className="truncate flex items-center gap-1.5">
            ...
            {chapter.type === 'summary' && <span className="text-base leading-none">📝</span>}
...
// Removed duplicates
  ...
  const [sidebarTab, setSidebarTab] = useState<'library' | 'study'>('library');
  ...
  const generateStudyPlan = async () => {
    ...
    setStudyPlan(null);
    try {
      const res = await fetch('/api/lessons/study-plan', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ docId: selectedDocId }) // Use selected doc, or null for all
      });
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex gap-4">
             <button 
                ...
                className={cn("text-[11px] font-display font-bold uppercase tracking-widest transition-colors", sidebarTab === 'library' ? 'text-cyan-400' : 'text-white/40 hover:text-white')}
             >
                Library
             </button>
             <button 
                onClick={() => setSidebarTab('study')}
                className={cn("text-[11px] font-display font-bold uppercase tracking-widest transition-colors", sidebarTab === 'study' ? 'text-cyan-400' : 'text-white/40 hover:text-white')}
             >
                Study Plan
             </button>
          </div>
      ...
      {sidebarTab === 'library' ? (
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col space-y-4">
           <p className="text-white/60 text-sm font-light leading-relaxed">
             Generate a personalized, multi-day study schedule based on {selectedDocId ? 'the selected document' : 'your entire library'}.
           </p>
           <button 
             onClick={generateStudyPlan}
             disabled={isGeneratingPlan}
             className="w-full bg-cyan-500 hover:bg-cyan-400 disabled:opacity-50 text-black py-2.5 rounded-lg font-bold text-sm transition-all"
           >
             {isGeneratingPlan ? 'Generating Schedule...' : 'Generate Study Plan'}
           </button>
           {studyPlan && (
